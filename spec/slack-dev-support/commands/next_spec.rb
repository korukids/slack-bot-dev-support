...
describe SlackDevSupport::Commands::Next do
  ...
  context 'when there are enough users to use the command next' do
    before do
      Redis.current.lpush(USERS_KEY, 'user_1')
      Redis.current.lpush(USERS_KEY, 'user_2')
      Redis.current.lpush(USERS_KEY, 'user_3')
    end

    it 'returns a prompt suggesting another user' do
      expect(message: "#{SlackRubyBot.config.user} next", channel: CHANNEL_NAME).to respond_with_slack_message("<@user_2> is on dev-support")
    end
  end
  ...
  context 'when there are not enough users to use the command next' do
    ...
    it 'reurns a message informing the user there are no more people to choose form' do
      expect(message: "#{SlackRubyBot.config.user} next", channel: CHANNEL_NAME)
